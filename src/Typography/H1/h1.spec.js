...
test('Render default H1 tag and styles', async () => {
  // Variables
  const minFS = 40;
  const maxFS = 60;
  const minLH = 42;
  ...
  const { getByTestId, container } = renderWithTheme(
    <H1 data-testid='test-text'>A tasty treat</H1>
  );
...
test('Render H1 tag with core props', async () => {
  // Render
  const { getByTestId, container } = renderWithTheme(
    <H1
...
test('Render H1 tag with fluid type props', async () => {
  // Variables
  const minFS = 60;
  const maxFS = 100;
  const minLH = 70;
