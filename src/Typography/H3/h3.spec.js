...
test('Render default H3 tag and styles', async () => {
  // Variables
  const minFS = 17;
  const maxFS = 27;
  const minLH = 27;
  ...
  const { getByTestId, container } = renderWithTheme(
    <H3 data-testid='test-text'>A tasty treat</H3>
  );
...
test('Render H3 tag with core props', async () => {
  // Render
  const { getByTestId, container } = renderWithTheme(
    <H3
...
test('Render H3 tag with fluid type props', async () => {
  // Variables
  const minFS = 60;
  const maxFS = 100;
  const minLH = 70;
