...
test('Render default Ol tag and styles', async () => {
  // Variables
  const minFS = 14;
  const maxFS = 17;
  const minLH = 17;
...
test('Render Ol tag with core props', async () => {
  // Render
  const { getByTestId, container } = renderWithTheme(
    <Ol
...
test('Render Ol tag with fluid type props', async () => {
  // Variables
  const minFS = 10;
  const maxFS = 18;
  const minLH = 18;
