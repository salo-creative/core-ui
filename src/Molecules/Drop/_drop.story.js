...
const stories = storiesOf('Molecules | Drop', module);
stories.addDecorator(withKnobs);
stories.addDecorator(withTests({ results }));
stories.addParameters({ jest: ['drop'] });
...
stories.add(
      ...
      <div>
        ...
        <ToggleVisibility store={ storeBasic }>
          <RenderWithProps store={ storeBasic }>
            <Drop
              open={ true }
              onClose={ () => storeBasic.set({ _STATE_VISIBILITY_TOGGLE_: false }) }
            >
              <Button fullWidth radius={ false } skin='text'>My account</Button>
              <Button fullWidth radius={ false } skin='secondary'>Login</Button>
            </Drop>
          </RenderWithProps>
        </ToggleVisibility>
      </div>
  ...
  { info: { propTablesExclude: [RenderWithProps, Button] }, notes: README }
);